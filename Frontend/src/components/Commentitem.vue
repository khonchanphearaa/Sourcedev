            ...
            <div class="flex-1 min-w-0">
                <div class="bg-paper-warm border border-paper-border rounded-2xl rounded-tl-sm px-4 py-3">
                    <div class="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span v-if="comment.author.role === 'admin'" class="text-xs bg-accent-light text-accent border border-accent/20 px-1.5 py-0.5 rounded-full font-sans font-semibold">Admin</span>
                    ...
                    <div v-else class="flex flex-col gap-2 mt-2">
                        <textarea v-model="editContent" rows="3" maxlength="1000" class="w-full px-3 py-2 border border-paper-border rounded-lg text-sm font-sans text-ink bg-white focus:outline-none focus:border-ink transition-colors resize-none leading-relaxed" @keydown.ctrl.enter="submitEdit" @keydown.meta.enter="submitEdit" />
                ...
                <div class="flex items-center gap-1 mt-1.5 px-1">
                    ...
                    <button v-if="!comment.parentComment && auth.isAuthenticated" @click="toggleReply" :class="['flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium font-sans transition-all duration-150 cursor-pointer',
                        ...
                        Reply
                        <span v-if="comment.replies?.length" class="text-ink-muted">({{ comment.replies.length }})</span>
                    </button>
                ...
                <Transition enter-active-class="transition duration-200 ease-out"
                    ...
                    <div v-if="replyOpen" class="mt-3 flex gap-3">
                        ...
                        <div class="flex-1">
                            <textarea v-model="replyContent" :placeholder="`Reply to ${comment.author.name}…`" rows="2" maxlength="1000" class="w-full px-3 py-2 border border-paper-border rounded-xl text-sm font-sans text-ink bg-white placeholder-ink-muted focus:outline-none focus:border-ink transition-colors resize-none leading-relaxed" 
                            @keydown.ctrl.enter="submitReply" @keydown.meta.enter="submitReply" ref="replyInputRef" />
                            <div class="flex items-center gap-2 mt-2">
                                <BaseButton size="sm" variant="accent" :loading="replying" :disabled="!replyContent.trim()" @click="submitReply">Reply</BaseButton>
...
const canDelete = computed(() => canEdit.value || auth.user?.role === 'admin')
